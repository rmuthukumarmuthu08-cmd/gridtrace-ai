#!/usr/bin/env python3
"""
Host-side generator for the Energy Loss Investigation testbed.
=============================================================
...
def main():
    ap = argparse.ArgumentParser(description="Generate labelled smart-meter telemetry (NDJSON).")
    ap.add_argument("--ticks", type=int, default=300,
                    help="frames per mode (1 frame = 1 simulated minute). Default 300.")
    ap.add_argument("--mode", choices=MODES + ["ALL"], default="ALL",
                    help="single mode, or ALL to cycle through every mode. Default ALL.")
    ap.add_argument("--seed", type=lambda x: int(x, 0), default=0xC0FFEE,
                    help="RNG seed. Change it for an independent validation set.")
    ap.add_argument("--load", type=float, default=1.0,
                    help="grid demand scaler 0.6-1.4 (the Wokwi slider). Default 1.0.")
    ap.add_argument("--out", default="-", help="output file, or - for stdout.")
    a = ap.parse_args()

    sim = Simulator(seed=a.seed, load=a.load)
    order = range(4) if a.mode == "ALL" else [MODES.index(a.mode)]
    fh = sys.stdout if a.out == "-" else open(a.out, "w", encoding="utf-8")

    try:
        fh.write(json.dumps({
            "record_type": "meta", "schema": "els.v1", "device": "PY-HOST-SIM",
            "feeder": "F01", "zones": ["Z01", "Z02"],
            "consumers": [c[0] for c in CONSUMERS],
            "tick_ms": 1000, "sim_seconds_per_tick": SIM_SECONDS_PER_TICK,
            "modes": MODES, "seed": a.seed, "grid_load_scale": a.load,
